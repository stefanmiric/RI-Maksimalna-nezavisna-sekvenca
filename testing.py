...
def compare(search_f1, search_f2, edges=10, nodes=10, iterations=100):
    print('Comparing {} and {}'.format(search_f1.__name__, search_f2.__name__))
    results = {
        search_f1.__name__: 0,
        search_f2.__name__: 0,
        'equal': 0
    }
    for i in range(0, iterations):
        graph = create_graph(edges, nodes)
        res_1 = search_f1(graph)[1]
        res_2 = search_f2(graph)[1]

        if res_1 < res_2:
            results[search_f1.__name__] += 1
        elif res_2 < res_1:
            results[search_f2.__name__] += 1
        else:
            results['equal'] += 1
    print(results)
...
def avg_results(functions, edges=10, nodes=10, iterations=10):
    ...
    plt.legend(list(map(lambda f: f.__name__ + ' ' + str(_averages[f.__name__]), functions)), loc='upper center', bbox_to_anchor=(0.5, -0.05))
...
def plot_results(function, edges= 10, nodes= 10, iterations = 100):
    ...
    plt.title("{} nodes / {} edges".format(nodes,edges))
